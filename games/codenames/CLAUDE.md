...
```bash
./build.sh              # Package plugin (requires pre-compiled ELF at export/bin/plugin.elf)
./build.sh compile      # Shows instructions for GDScript→ELF compilation via godot-sandbox
```

The build output is `export/codenames.daccord-plugin` (a ZIP bundle containing `plugin.json`, the compiled ELF, and assets).

There are no tests or linting tools configured.

## Architecture

**Single-file game**: All logic lives in `src/main.gd` — a GDScript class that the daccord ScriptedRuntime hosts. There is no scene tree; everything is drawn imperatively via a bridge API dictionary (`api`).
...
**Plugin metadata**: `plugin.json` defines the plugin ID, canvas size (640×480), max 8 participants, unlimited spectators, and required permissions.
